...
import { AGENTS, CHANNEL_LABEL } from "@/data/agents";
          ...
          <div className="lg:col-span-7 flex flex-col" role="tablist" aria-label="Postes du catalogue">
            {AGENTS.map((ag, i) => (
              ...
              >
                ...
                <span className="mono text-[13px] text-right text-muted">{PRICE_PER_AGENT}{NB}€{NB}HT / mois</span>
              </button>
            ))}
            <a href="#tarifs" className="arow arow-plus text-accent" style={{ cursor: "pointer" }}>
              <span className="num text-accent">+</span>
              <span className="text-[17px] md:text-[18px] font-medium">Votre poste</span>
              <span className="hidden md:block text-[14px] text-muted">Un besoin qui n&apos;est pas dans la liste{NB}? On le développe.</span>
              <span className="mono text-[13px] text-right">sur devis</span>
            </a>
          </div>

          {/* Fiche */}
          <div id="agent-fiche" role="tabpanel" className="box lg:col-start-8 lg:col-span-5 p-6 md:p-7 flex flex-col gap-4 md:gap-[18px]">
            ...
            <h3 className="h3" style={{ fontSize: "clamp(24px, 2.2vw, 30px)" }}>{a.headline}</h3>
